...
```{r}
beamarrival <- beam %>% filter(type == "TripArrivalEvent") %>% 
  group_by(legMode) %>%
  summarize(modeNum = n())
```
...
```{r, warning=FALSE, message=FALSE}
tracerun <- read_csv("events/slc-light-2.5k-tpcm-hov-test-all__2022-01-28_06-17-02_mhw/ITERS/it.1/1.events.csv", col_types = cols(.default = "c")) %>%
  filter(type == "ModeChoice") %>%
  group_by(mode) %>%
