...
asim_asc <- tail(as_asc,-33) %>% 
  ...
  filter(alternative %in% c("bike","drive_transit","tnc_single","tnc_shared","walk","walk_transit","sr2","sr3p")) %>%
...
asim_asc[asim_asc == "sr2"] <- "hov2"
asim_asc[asim_asc == "sr3p"] <- "hov3"
```
...
alts <- c("bike","bike_transit","car","drive_transit","ride_hail","ride_hail_pooled","ride_hail_transit","walk","walk_transit","hov2","hov3")
vals <- c(1:11)
...
costAlts <- c(2,3,4,5,6,7,9,10,11)
vehicleTimeAlts <- c(2,3,4,5,6,7,9,10,11)
waitTimeAlts <- c(2,4,5,6,7,9)
egressTimeAlts <- c(2,3,4,5,6,7,9,10,11)
...
renamed_mtc <- mtc_cost %>%
  ...
  mutate(variable = case_when(
    ...
    Variable == "age16P" ~ Variable,
    Variable == "hhsize1" ~ "hhSize1",
    Variable == "hhsize2" ~ "hhSize2",
  ...
  mutate(alternative = list(c(1:11))) %>%
  unnest(alternative) 
...
path_coeff <- renamed_mtc %>% pivot_longer(!c(variable,Variable, alternative), names_to = "latentClass", values_to = "value") %>%
  mutate(value = case_when(
    ...
    variable == "age16P" & alternative %!in% age16PAlts ~ 0,
    variable == "hhSize1" & alternative %!in% hh1Alts ~0 ,
    variable == "hhSize2" & alternative %!in% hh2Alts ~ 0,
...
```{r}
path_variables <- c("vehicleTime","waitTime","egressTime","transfer","shortWalkDist","longWalkDist","shortBikeDist","longBikeDist","originCloseToTransit","originFarFromTransit","destCloseToTransit","destFarFromTransit","shortDrive","walkTime")
person_variables <- c("ascAutoDeficient","ascAutoSufficient","ascNoAuto","cost","age1619","age010","age16P","hhSize1","hhSize2")
location_variables <- c("ZTI","destZDI","originZDI","CBD")

asim_vars <- c("vehicleTime","waitTime","egressTime","originCloseToTransit","originFarFromTransit","destCloseToTransit","destFarFromTransit","transfer","shortWalkDist","longWalkDist","shortBikeDist","longBikeDist","cost","ZTI","destZDI","originZDI","age1619","age010","age16P","hhSize1","hhSize2","shortDrive","CBD","ascNoAuto","ascAutoDeficient","ascAutoSufficient","walkTime")
```
...
asim_long <- rbind(asim_asc_complete,path_cf) %>%
  ...
  mutate(
    model = "modeChoice", 
    #double path variables to accound for BEAM using trips not tours
    value = ifelse(variable %in% path_variables | variable == "cost",value*2,value), 
    #replace 0s with blank values to make utility function simpler
...
teleport <- asim_long %>% filter(alternative %in% c("hov2","hov3")) %>%
  mutate(alternative = ifelse(alternative =="hov2","hov2_teleportation","hov3_teleportation"))
asim_long <- rbind(asim_long,teleport) %>%
  arrange(factor(variable, levels = asim_vars),alternative,factor(latentClass, levels = purps))
```

```{r}
# lets add a blank walkTime variable too
walkTime <- asim_long %>% filter(variable == "vehicleTime") %>%
  mutate(variable = "walkTime", value = "")
asim_long_R <- rbind(asim_long,walkTime)

write_csv(asim_long_R,"asim-long-R-blank.csv")
```

## Adjusted Complete File
Dr. Erhardt wants to adjust the some of the values in the current version of the asim-long file to ensure uniformity in the equation for vehicle time, wait time, walk time, and number of transfers. The equation he wants is shown below.

\begin{equation}
  WeightedTravelTime = InVehicleTime + 2(WaitTime) + 2(WalkTime) + 10(NumTransfers)
\end{equation}

Below, we tweek the values in the current version to ensure that these variables have constant coefficients. In addition, we delete the longWalk variable. Actually, we leave all other variables blank.


```{r}
vehicleVals <- asim_long %>%
  filter(variable == "vehicleTime") %>%
  mutate(comName = paste(alternative,latentClass, sep = "_"), vehicleValue = value) %>%
  select(8,9)

adj_asim <- asim_long %>%
  filter(variable %in% c("vehicleTime","waitTime","shortWalkDist","transfer")) %>%
  mutate(variable = ifelse(variable == "shortWalkDist","walkTime",variable)) %>%
  mutate(value = ifelse(variable == "walkTime",value*.05,value)) %>%
  mutate(comName = paste(alternative,latentClass, sep = "_")) %>%
  left_join(vehicleVals, by = "comName") %>% select(-comName) %>%
  mutate(vehicleValue = ifelse(is.na(value),NA,vehicleValue)) %>%
  mutate(ratio = value/vehicleValue) %>%
  mutate(adjValue = case_when(
    variable == "waitTime" ~ 2 * vehicleValue,
    variable == "shortWalkDist" ~ 2 * vehicleValue,
    variable == "transfer" ~ 10 * vehicleValue
  )) %>%
  mutate(newRatio = adjValue/vehicleValue)
```


```{r}
write_csv(asim_long_adj,"asim-long-R-adj.csv")
```
