...
## Path Coefficient Data
### Reading in the Data
First, lets read in the path variable coefficient values from the MTC 2012 release draft. In addition, we use the time values from the MTC 2012 release draft to estimate a cost coefficient. This is done in the following code, and then attached to the mtc data frame.

```{r, message=FALSE, warning=FALSE}
mtc2012 <- read_table2("model_data/mtc/mtc_2012_tour_mode_choice_coefficients_path.txt")
cols <- sapply(mtc2012, is.numeric)
mtc2012[, cols] <- mtc2012[, cols] * 2
...
### Creating a Path Coeff Dataframe
...
```{r}
path_variables <- c("vehicleTime","waitTime","egressTime","transfer","shortWalkDist","longWalkDist","shortBikeDist","longBikeDist","originCloseToTransit","originFarFromTransit","destCloseToTransit","destFarFromTransit")
person_variables <- c("ascAutoDeficient","ascAutoSufficient","ascNoAuto","cost")
```
...
asim_long <- rbind(asim_asc_complete,path_cf) %>%
  arrange(variable,alternative,factor(latentClass, levels = purps)) %>%
  mutate(
    model = "modeChoice", 
    tourType = case_when(
