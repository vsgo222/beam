    ...
    describe("Run with multiple persons") {
      ...
      runWithConfig(
        "test/input/beamville/beam-urbansimv2.conf",
        {
          ...
          case e: PathTraversalEvent if e.currentTourMode.contains("hov3") && e.mode == BeamMode.CAR =>
            carHov3passengers.add(e.numberOfPassengers)
          case e: PathTraversalEvent if e.currentTourMode.contains("hov2") && e.mode == BeamMode.CAR =>
