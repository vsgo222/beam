...
object UtilityFunctionOperation {
  ...
  def apply(s: String, value: Double): UtilityFunctionOperation = {
    (s, value) match {
      ...
      case ("cost", _)           => Multiplier(value) // for LCCM use
      case ("time", _)           => Multiplier(value) // for LCCM use
      case ("vehicleTime", _)    => Multiplier(value) // for TPCM use
      case ("waitTime", _)       => Multiplier(value) // for TPCM use
