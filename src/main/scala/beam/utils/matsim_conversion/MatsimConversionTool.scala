...
object MatsimConversionTool extends App {
  ...
  def readInConfig(cf: String): Unit ={
    if (null != cf && cf.nonEmpty) {
      val beamConfigFilePath = cf //"test/input/beamville/beam.conf"
      println(beamConfigFilePath)
      ...
      // generateTazDefaults cannot be jarred due to dependency conflicts. Comment it out for jarring
