...
setwd("~/RA_Microtransit/beam/activitysim_output")
...
#Calculate number of households to keep
zoneRows <- nrow(householdsInZone) * pctInZone / 100
noZoneRows <- nrow(householdsOutZone) * pctOutZone / 100

#Create and combine sample households
sampleHouseholds <- rbind(householdsInZone[1:zoneRows,],householdsOutZone[1:noZoneRows,])
